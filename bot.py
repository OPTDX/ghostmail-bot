...
# -----------------------------
# GhostMail ✉️ (Render Web Service / Webhook Mode)
# User actions via attractive buttons:
#   Add New ⭐  -> /new
#   Inbox 📥   -> /inbox
#   Delete 🚫  -> /delete
#   Copy Email 📋 -> shows current email address
# Minimal commands still work: /start /new /inbox /delete
# Admin-only: /stats, /broadcast
# -----------------------------
...
# Render public URL, e.g. https://ghostmail.onrender.com
RENDER_EXTERNAL_URL = os.environ.get("RENDER_EXTERNAL_URL", "").strip()

# Render provides PORT
PORT = int(os.environ.get("PORT", "10000"))

# Notifications (recommended OFF on free web service)
...
async def delete_user_message(update: Update):
    # Deletes user messages (commands + button presses) to keep DM clean
    try:
        if update.message:
...
async def send_clean(chat_id: int, app: Application, text: str, *, parse_mode=None, reply_markup=None, disable_preview=True):
    # Keep DM clean: remove previous bot message, then send new one
...
async def new_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show email clearly (easy to tap+hold copy)
    await send_clean(
        chat_id=user_id,
        app=context.application,
...
async def copy_email_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not inbox:
        await send_clean(
            chat_id=user_id,
            app=context.application,
            text="No temp email yet. Tap Add New ⭐",
            reply_markup=user_menu(has_inbox=False),
        )
        return
...
async def inbox_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if mid in inbox.seen_message_ids:
            ...
            from_name = full.get("from", {}).get("name") or ""
...
async def delete_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_clean(
        chat_id=user_id,
        app=context.application,
        text="🗑️ Deleted your temp email.\nTap Add New ⭐ to create another.",
        reply_markup=user_menu(has_inbox=False),
    )


# ---------------- Menu text router (for attractive buttons) ----------------
async def menu_text_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Unknown text: keep clean and show menu again
    await delete_user_message(update)
    has_inbox = bool(get_inbox(update.effective_user.id))
    await send_clean(
        chat_id=update.effective_user.id,
