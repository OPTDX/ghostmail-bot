...
#
# ✅ Root URL (/) returns: "GhostMail Bot is running ✅"
# ✅ Also /health returns the same (for monitors)
#
# Attractive menu buttons:
#   New Email 📨     -> create temp email
#   Inbox 📥      -> newest email only (subject bold + body quote) + From
#   Delete 🚫     -> delete temp email
#   Copy Email 📋 -> show current email
#
# DM clean:
#   - deletes user messages (commands + menu button presses)
#   - deletes previous bot message before sending a new one
#
# Admin-only:
#   /stats
#   /broadcast <text> (or reply then /broadcast)
# ============================================================
...
# IMPORTANT: set this in Render env to your public URL, e.g. https://ghostmail.onrender.com
...
async def new_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_clean(user_id, context.application, "Creating your temp email…", reply_markup=user_menu(has_inbox=False))
...
async def inbox_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        # delete all emails from server to keep clean
        for m in msgs:
            did = m.get("id")
            if did:
                try:
...
def main():
    ...
    # Attach health routes to the aiohttp app used by PTB
    web_app = web.Application()
    web_app.router.add_get("/", health_root)
